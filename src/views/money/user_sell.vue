  ...
  <div>
    ...
    <div class="container">
      <el-table :data="tableData.slice((query.pageIndex-1)*query.pageSize,query.pageIndex*query.pageSize)" border class="table" ref="multipleTable" header-cell-class-name="table-header">
        ...
        <el-table-column prop="batch_id" label="药品批次ID"></el-table-column>
        <el-table-column prop="sold_num" label="购买数量"></el-table-column>
        <el-table-column prop="sold_sum" label="购买金额"></el-table-column>
        <el-table-column prop="sold_time" label="购买时间">
          <template #default="scope">
            {{dateFormat(scope.row,"sold_time")}}
          </template>
        </el-table-column>
...
export default {
  ...
  setup() {
    ...
    const getData = () => {
      BillHelper.getSellInfo().then(info=>{
        if(info.code===200){
