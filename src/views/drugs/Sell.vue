...
export default {
  ...
  setup() {
    ...
    const getData = () => {
      BillHelper.getItems_admin().then(info=>{
        if(info.code===200){
      ...
      ElMessageBox.confirm("确定要删除吗？", "提示", {
        type: "warning",
          ...
          .then(() => {
            ...
            BillHelper.delitem_admin(params).then((info)=>{
              if(info.code===200){
    ...
    const buy = () => {
        BillHelper.setSell_admin().then((info)=>{
          if(info.code===200){
    ...
    const addInfo = () => {
      ...
      if(add_form.purchase_num<add_form.sold_num){
        ...
        BillHelper.addItem_admin(params).then((info) => {
          //以下是响应部分
          if (info.code === 200) {
